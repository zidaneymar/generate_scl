...
class Generator:
    ...
    def generate_get_input_for_basic_type(node, env):
        if isinstance(node, Entity):
            ...
            res = Generator.say("Please enter a type of '%s', see description: '%s'" % (node.type, node.description))
            res += "\r\nGET_INPUT\r\n"
            new_variable = Variable(node)
            new_variable.add_to_env(env)
            ...
            if not node.is_unit_type() and not node.is_disabled():
                for entity in node.sub_entities:
                    if entity.is_unit_type():
                        res += Generator.generate_get_input_for_basic_type(entity, env)
                    else:
                        res += Generator.say("Please input details of this custom entity name: %s, type: %s, description: %s" % (entity.name, entity.type, entity.description))
                        # before_index = len(env)
                        res += Generator.generate_get_input(entity, env)
                        # After gnerate all children types, creating a new complex type
                        #res += generate_declaration(entity, env)
                res += Generator.generate_declaration(node, env)
                return res
            elif node.is_unit_type():
                res += Generator.generate_get_input_for_basic_type(node, env)
                return res
            else:
                return res
    @staticmethod
    def generate_call(service, action, env):
        if isinstance(action, Action) and isinstance(service, Service):
            ...
            for input_param in action.input_params:
                res += Generator.generate_get_input(input_param, env)
            ...
            res += "\r\n"
            res += Generator.generate_call_error(token_map)
            res += Generator.generate_call_result(token_map)
            return res
            #for input_param in action.input_params:
    @staticmethod
    def generate_call_result(token_map):
        ...
        res += Generator.generate_feedback(token_map)
        return res
    @staticmethod
    ...
    def generate_feedback(token_map):
        res = Generator.say("Is this action working properly?")
        ...
        res += Generator.say("We have received your feedback: ${log}")
        res += "GO start\r\n"
        return res
    ...
    def generate_actions_choices(service):
        if isinstance(service, Service):
            res = Generator.say("Please choose an action from below:")
            action_names = []
            for action in service.actions:
                res += Generator.create_button(action.name, action.name, action.name)
                action_names.append(action.name)
            res += Generator.render(action_names)
            res += Generator.get_input()
            for action in service.actions:
                res += Generator.scl_if("USER_INPUT == \"%s\"" % action.name, "GO %s" % action.name)
            return res
    
    @staticmethod            
    def generate_feedback_buttons():
        res = ""
        res += Generator.create_button("Yes", "Yes", "yes_button")
        res += Generator.create_button("No", "No", "no_button")
        res += Generator.create_button("Not sure", "Not sure", "not_sure_button")
        return res

    @staticmethod
    def create_bot():
        
    def generate(organization_name):
        tools = Tools()
        env = Environment()
        for service in tools.get_all_service_under_org(organization_name):
            #print str(service)
            print Generator.generate_feedback_buttons()
            print Generator.generate_actions_choices(service)

            for action in service.actions:

                print "%s:\r\n" % action.name
                print  Generator.say("Now we are testing action, action name: %s, action description: %s" % (action.name, action.description))
                #print str(action)
                #print action.name
                print Generator.generate_call(service, action, env)
